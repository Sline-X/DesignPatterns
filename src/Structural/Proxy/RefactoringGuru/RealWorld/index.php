 ...
 *
 * Дело в том, что у большинства прилолжений нет чётко определённого интерфейса.
...
class SimpleDownloader implements Downloader
{
    public function download(string $url): string
    {
        ...
        echo "Download bytes: " . strlen($result) . "\n";
...
/**
 * Клиентский код может выдать несколько похожих запросов на загрузку. В
 * этом случае кэширующий заместитель экономит время и трафик, подавая
 * резуьтаты из кэша
 *
 * Клиент не знает, что он работает с хаместителем, потому что он работает с
